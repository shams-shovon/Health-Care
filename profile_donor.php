...
$sql = "SELECT * FROM donor where id = '".$_SESSION['uname']."'";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
       ...
       $userarray["blood_group"] =  $row['blood_group'];
       $userarray["last_donate_date"] =  $row['last_donate_date'];
...
function lbdd(){
	var un=document.frm.lbday.value;
	if(un.length<3){
		document.frm.lbday.style.color="red";
		document.getElementById("msg").innerHTML="invalid last donation Day";
	}
	else{
		document.frm.lbday.style.color="black";
		document.getElementById("msg").innerHTML="valid last donation Day";
	}
...
function validate(){
	...
	var lbday=document.frm.lbday.value;
	...
	else if(lbday.length==0){
		flag=false;
		str="must choose last donation Day";
	}
...
<li><a  href="Donor_home_page.html"> home </a></li>
<li><a href="#"Home</a></li>
<li><a href="#"Home</a></li>
<li><a href="#"Home</a></li>
<li><a href="#"Home</a></li>
...
<li><a href="#"Home</a></li>
<li><a  href="Donor_home_page.html">back </a></li>
</ul>
</nav>
<form id ="frm" style= "color:DodgerBlue"   action="updatedonor.php" method="post" name="frm" >
...
<?php 
$a = $userarray['blood_group'];
?>
BloodGroup:
 <select name="bloodgroup">
    <option  value="A+" <?php echo($a == "A+")? 'selected':''; ?>>A+</option>
    <option  value="A-" <?php echo($a == "A-")? 'selected':''; ?> >A-</option>
    <option  value="AB+" <?php echo($a == "AB+")? 'selected':''; ?>>AB+</option>
    <option  value="AB-" <?php echo($a == "AB-")? 'selected':''; ?>>AB-</option>
	<option  value="B+" <?php echo($a == "B+")? 'selected':''; ?>>B+</option>
    <option  value="B-" <?php echo($a == "B-")? 'selected':''; ?>>B-</option>
	<option  value="O+" <?php echo($a == "O+")? 'selected':''; ?>>O+</option>
	<option  value="O-" <?php echo($a == "O-")? 'selected':''; ?>>O-</option>
  </select>
  <br><br>
  
Last Blood Donation Date : <input onkeyup="lbdd()" type="date" name="lbday" value="<?php echo $userarray['last_donate_date']?>" id="txt"><span id="msg"></span> <br><br>
Phone No:
...
<input type="submit" onclick="return validate()" name="sbt" value="submit" /></br></br></br>
...
if(isset($_SESSION["uname"])){?>
	...
	<a href="picture_donor.php">Change Photo</a><br/>
