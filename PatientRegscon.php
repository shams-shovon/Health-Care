...
<?php
$file=fopen('file_patient.txt','a') or die("file open error");
$c=0;
$confirm = 0;
...
if (strlen($_REQUEST["pblm"])==0)
{
	echo "<p style= 'color:red'> Must input your problem</p>";
}
...
if($_REQUEST["pass"]==$_REQUEST["confpass"]  &&  strlen($_POST["phn"])== 11 && $confirm == 0){
	...
	$conn = mysqli_connect("localhost", "root", "","web_tech");
		$a =rand(100,1000)."-p-".rand(100,1000);
		$d = "patient";
		
		$sql = "INSERT INTO `patient`( `id`, `first_name`, `last_name`, `address`, `dob`, `gender`, `problem`,`phone`, `email`, `password`) VALUES('".$a."','".$_POST["firstname"]."','".$_POST["lastname"]."', '".$_POST["address"]."', '".$_POST["bday"]."','".$_POST ["gender"]."', '".$_POST ["pblm"]."','".$_POST ["phn"]."','".$_POST ["email"]."','".md5($_POST["pass"])."');";
